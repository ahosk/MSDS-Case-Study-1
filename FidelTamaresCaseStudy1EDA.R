...
#Also imported USStatePopluation and 3 year growth, did not use code, manually imported becuase 
#Rstudio was importing values as char
...
completebrewerylist2 = merge(completebrewerylist1, USPop2019OnlyStates3yrGrowth, by="State", all.x=TRUE)
...
ggplot(data=completebrewerylist1,aes(x=ABV))+geom_histogram(aes(y=..density..), colour="black", fill="white")+geom_density(colour="red", size=1) + labs(title="ABV Distribution")
...
#top 5 cities with the most beer styles
...
statecount = aggregate(completebrewerylist2$State,by=list(completebrewerylist2$State),length)
colnames(statecount) = c("region","count")
statecount$region = tolower(state.name[match(trimws(as.character(statecount$region)),state.abb)])
            
ggplot(statecount, aes(map_id = region))+
...
#population by state
ggplot(USPop2019OnlyStates3yrGrowth1,aes(x=reorder(sabb,pop2019), y=pop2019, fill = sabb)) +
...
#adding breweries per state to from question 1
#updating dataset to include per capita

statecount1 <- statecount
names(statecount1)[1] = "State_Name"
statecount2 = merge(statecount1, stateinfo, by= "State_Name", all.x=TRUE)
statecount3 = merge(statecount2, USPop2019OnlyStates3yrGrowth, by="State", all.x=TRUE)
statecount3["BP100K"] = round(((statecount3["count"]/statecount3["2019pop"]) * 100000), digits = 1)

#adjusting variable names
USPop2019OnlyStates3yrGrowth2 <- USPop2019OnlyStates3yrGrowth
USPop2019OnlyStates3yrGrowth3 = merge(USPop2019OnlyStates3yrGrowth2, statecount3, by="State", all.x=TRUE)
...
#Plotting brewers per capita (100k) REVERSED
USPop2019OnlyStates3yrGrowth3 %>%
  na.omit() %>%
  ggplot(aes(x=reorder(sabb,sabb), y=BP100K, fill = sabb)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label=BP100K), position=position_stack(vjust=0.5), size=3) +
  coord_flip() +
  ggtitle("Breweries per 100k") +
...
#Population per state REVERSED
ggplot(USPop2019OnlyStates3yrGrowth1,aes(x=reorder(sabb,sabb), y=(pop2019/1000000), fill = sabb)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  ggtitle("Population by State") +
  labs(x="States", y="Population (in millions)", colour="States") +
  theme(legend.title=element_blank()) + 
